...
name_mapper = c(
  ...
  # renamed SPSS variables from neuropsych_tests instrument
...
data <- data %>% 
  mutate(recode_variable = recode(variable, !!!name_mapper),
         order = 1:length(recode_variable),
         tp = str_match(recode_variable, "_([:digit:]|[bB][lL])$")[, 2],
         tp = str_to_lower(tp),
         type = case_when(
            str_detect(recode_variable, "_raw_") ~ "raw",
            str_detect(recode_variable, "_ss_") ~ "ss",
            str_detect(recode_variable, "_perc_") ~ "perc",
            TRUE ~ "other"),
         base = str_remove(recode_variable, "_([:digit:]|[bB][lL])$"),
